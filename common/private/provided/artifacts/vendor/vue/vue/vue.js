/**
Slightly customized version of vue, changes are:

V allow vm on document.body (line 10881)
X expose the observe method in the public API to build services with observable state (line 4063) (no longer needed)
V updated $emit to check for events {} like in vue1 (line 2562)
X line 406: check that not undefined (should check wherever it is being called but not sure which line is causing issues) (fixed by vue itself)
V line 1928: disable proxy when doing server-side rendering, it is not correctly implemented in htmlunit
V expose initWatch and initComputed in util, need to be able to initialize versions that wait for asynchronous data to be loaded (line 5062)
	V line 3450: make sure we don't overwrite the computed watchers because we now have a two-phase computed property
V 5343: added function decodeCustom to decode our encoded single quotes (through translations) because they can appear anywhere, including in interpreted attributes with ternaries
