<template id="connection">
	<div class="connection" v-auto-close="function() { showing = false }">
		<button v-show="connections.length" @click="showing = !showing" class="info"><span class="n-icon n-icon-database"></span><span class="content">{{ $services.manager.connection() }}</span></button>
